...
void speed_data_init(speed_data *data) {
    ...
    // stopping data
    data->stopping_data[0] = 135168; // tr 24
    data->stopping_data[1] = 127966; // tr 58
...
int32_t get_stopping_distance(speed_data *data, uint16_t n) {
    return data->stopping_data[trn_hash(n)];
}
...
int32_t get_distance_from_velocity(speed_data *data, uint16_t trn, int32_t time, uint16_t s) {
    /*
    *
    *   Formula: t_clock_ticks * v / 100 = d
    *
    */
   int32_t velocity = get_velocity(data, trn, s);

   return (velocity * time) / 100;
}
